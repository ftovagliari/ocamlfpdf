  ...
  begin
    try
      ...
      let print_page key font =
        ...
        let markup = Fpdf_markup.prepare ~width:width_avail ~padding ~markup:characters doc in
        markup.Fpdf_markup.print ~x:x0 ~y ();

        let y = y +. markup.Fpdf_markup.height in
        Fpdf.set_font ~family:font.fontFamily ~size:18. ~style:(Font.style font) doc;
        let markup = Fpdf_markup.prepare ~width:width_avail ~padding ~markup:sample doc in
        markup.Fpdf_markup.print ~x:x0 ~y ();

        let y = y +. markup.Fpdf_markup.height in
        Fpdf.set_font ~family:font.fontFamily ~size:11. ~style:(Font.style font) doc;
        let markup = Fpdf_markup.prepare ~width:width_avail ~padding ~markup:sample_long doc in
        markup.Fpdf_markup.print ~x:x0 ~y ()
      in
