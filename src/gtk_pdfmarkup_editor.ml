...
class editor ~buffer ~size ?shadow_type ?packing () =
  ...
  let button_fgcolor    = GButton.color_button ~color:(GDraw.color `BLACK) ~title:"Colore del testo" ~packing:toolbox#pack ~show:false () in
  ...
  let sw                = GBin.scrolled_window ~width:500 ~height:300 ~hpolicy:`AUTOMATIC ~vpolicy:`AUTOMATIC ~packing:vbox#add ?shadow_type () in
...
object (self)
    ...
    method set_markup markup =
      ...
      Xml.iter begin function
        ...
        | Xml.Element (tag, attrs, children) when (String.lowercase tag) = "span" ->
          ...
          in
          (**  *)
          let tags = filter_map (fun x -> x) (size :: underline :: styles) in
          begin
            match children with
              | [] -> buffer#insert ~tags " "
              | children ->
    ...
    method get_markup () =
      ...
      while not !iter#is_end do
        ...
        (**  *)
        current_uline := List.exists begin fun t ->
          t#get_oid = tag_uline#get_oid
        end !current_tags;
        (**  *)
