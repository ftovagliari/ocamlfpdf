...
let header_draw ~columns ~nodes ~x ~y ~padding ?align ~line_disjoin ~border_width doc =
  ...
  let line_height = (FPDF.font_size doc) /. FPDF.scale doc in
  let rec draw_node level x y = function
    | `Leaf id ->
      ...
      if col.col_width > 0. then begin
        ...
        FPDF.set ~x:(x +. padding) ~y:(y +. padding) doc;
        let height =
          match col.col_title with
            | `Text text ->
              FPDF.multi_cell ~width ~line_height ~border ?align ~text doc;
  ...
  let points = FPDFUtil.group_by (fun (x, _, _) -> x) !points in
  ...
  with_line_width doc 0.0 begin fun () ->
    ...
    List.iter begin fun (x', (y', lw)) ->
      if lw < max_float && x' < !x -. lw then begin (* the right-most line is on the right table border *)
        ...
        FPDF.set_line_width lw doc;
        FPDF.line ~x1:x' ~y1:(y' +. lw/.2. +. line_disjoin) ~x2:x' ~y2:(y +. height -. lw/.2.) doc;
      end;
    end points;
  end ();
  height, points
...
let print
    ...
    ?caption
    doc =
  let margin_top, _, _, _ = FPDF.margins doc in
  let has_border = has_border border in
  let with_line_width thickness f x =
    let old = FPDF.line_width doc in
    FPDF.set_line_width (size_of_thickness thickness) doc;
    f x;
    FPDF.set_line_width old doc
  in
    ...
    begin
      match grid_lines with `None -> () | _ ->
        ...
        List.iter begin fun (_, col) ->
          FPDF.line ~x1:(!xx +. line_disjoin) ~y1:yy ~x2:(!xx +. col.col_width -. line_disjoin) ~y2:yy doc;
          xx := !xx +. col.col_width;
        end columns;
    end;
    FPDF.set_font ~style:[] doc;
    FPDF.set ~x:!x ~y:!y doc;
    height, points
  in
  (** Caption *)
  begin
    match caption with None -> () | Some caption ->
      ...
      FPDF.set_font ~style:[`Bold] ~size:(size *. 1.2) doc;
      FPDF.set ~x:!x ~y:!y doc;
      let line_height = (FPDF.font_size doc) /. FPDF.scale doc in
      FPDF.multi_cell ~width ~line_height ~border:[] ~align:`Center ~text:caption doc;
      FPDF.set_font ~style:[] ~size doc;
  ...
  let print_vertical_lines ~x ~y ~bottom () =
    let xw = x +. width in
    if has_border `T then with_line_width border_width (FPDF.line ~x1:x ~y1:y ~x2:xw ~y2:y) doc;
    if has_border `L then with_line_width border_width (FPDF.line ~x1:x ~y1:y ~x2:x ~y2:bottom) doc;
    if has_border `B then with_line_width border_width (FPDF.line ~x1:x ~y1:bottom ~x2:xw ~y2:bottom) doc;
    if has_border `R then with_line_width border_width (FPDF.line ~x1:xw ~y1:y ~x2:xw ~y2:bottom) doc;
      ...
      with_line_width `Thin begin fun () ->
        ...
        List.iter begin fun (x', (y', lw)) ->
          if lw > 0. && lw < max_float && x' < x +. width then begin (* the right-most line is on the right border *)
            ...
            FPDF.set_line_width lw doc;
            FPDF.line ~x1:x' ~y1:(y +. lw/.2.) ~x2:x' ~y2:(bottom -. lw/.2. -. line_disjoin) doc;
          end
        end !vlines_points;
      end ()
    | _ -> ()
  ...
  let (!!) = FPDFUtil.memo ~f:(list_pos tags) in
  let index = ref 0 in
  let cont = ref [] in
  ...
  List.iter begin fun row ->
    ...
    let heights = List.map begin fun (tag, col) ->
      try
        ...
        incr i;
        maxh
      with Not_found -> failwith "FPDFTable: no such column"
    end columns in
    ...
    if !y -. !top +. row_height +. rowspacing > !y_avail then begin
      ...
      top := !y;
      let margin_top, _, _, _ = FPDF.margins doc in
      y_avail := margin_top +. page_header_height +. page_height -. FPDF.y doc;
      let hh, vp = print_titles ~x ~y () in
      vlines_points := vp;
      header_height := hh;
      x := !left;
                ...
                match properties.prop_image with
                  | None ->
                    ...
                    if use_markup then begin
                      let markup = FPDFMarkup.prepare ~width:(width -. 2. *. cellpadding)
                          ~line_spacing ~markup:properties.prop_text doc in
                      markup.FPDFMarkup.print ~x:(x +. cellpadding) ~y ()
                    end else begin
    ...
    begin
      match grid_lines with
        | `Horizontal | `Both ->
          ...
          let f = fun () -> FPDF.line ~x1 ~y1 ~x2 ~y2 doc in
          cont := f :: !cont;
        | _ -> ()
    end;
