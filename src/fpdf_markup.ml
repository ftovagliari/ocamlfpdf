...
let markup_to_blocks ~markup ~line_spacing doc =
  ...
  let attr doc = {
    ...
    rise         = None;
    underline    = `NONE;
    color        = (Fpdf_util.hex_of_rgb (Fpdf.text_color doc));
    bgcolor      = None;
    align        = 0.0;
  ...
  Xml.iter begin function
    ...
    | Xml.Element (tag, attrs, children) when (String.uppercase tag) = "SPAN" ->
      let attr     = {
        ...
        color        = (try List.assoc "color" attrs with Not_found -> Fpdf_util.hex_of_rgb (Fpdf.text_color doc));
...
let prepare ~width ~markup ?bgcolor ?border_width ?border_color ?border_radius ?(padding=(0., 0., 0., 0.)) ?(line_spacing=1.) (*?pre*) doc =
  ...
  analysis.print <- begin fun ~x ~y ?valign () ->
    ...
    if border_width <> None then (Fpdf_graphics_state.pop doc)
  end;
  analysis
;;
