...
module App_config = struct let (//) = Filename.concat let (!!) = Filename.dirname let application_param = try List.fold_left begin fun acc x -> match Str.split (Str.regexp "=") x with | n :: v :: [] -> (n, v) :: acc | n :: [] -> (n, "") :: acc | _ -> acc end [] (Str.split (Str.regexp ",") (Sys.getenv "OCAMLEDITORPARAM")) with Not_found -> [];; let application_debug = try (List.assoc "debug" application_param) = "2" with Not_found -> false;; let get_application_path name =  let base = !! (!! Sys.executable_name) in let path = if base = "." || base = (!!(Sys.getcwd())) then ".." // name else base // "share" // name in let install_path = base // "share" // "ocamleditor" // name in if Sys.file_exists install_path then install_path else path let application_icons = get_application_path "icons" let application_plugins = get_application_path "plugins" end
module Ocaml_config = struct open Printf let redirect_stderr = if Sys.os_type = "Win32" then " 2>NUL" else " 2>/dev/null" let putenv_ocamllib value = match Sys.os_type with | "Win32" -> let value = match value with None -> "" | Some x -> x in Unix.putenv "OCAMLLIB" value | _ -> ignore (Sys.command "unset OCAMLLIB") let find_best_compiler compilers = try Some (List.find begin fun comp -> try ignore (kprintf Cmd.expand "%s -version%s" comp redirect_stderr); true with _ -> false end compilers) with Not_found -> None;; let find_tool which path = let commands = match which with | `BEST_OCAMLC -> ["ocamlc.opt"; "ocamlc"] | `BEST_OCAMLOPT -> ["ocamlopt.opt"; "ocamlopt"] | `BEST_OCAMLDEP -> ["ocamldep.opt"; "ocamldep"] | `BEST_OCAMLDOC -> ["ocamldoc.opt"; "ocamldoc"] | `OCAMLC -> ["ocamlc"] | `OCAML -> ["ocaml"] in let quote    = if path <> "" && Sys.os_type = "Win32" && String.contains path ' ' then Filename.quote else (fun x -> x) in let path     = if path <> "" then Filename.concat path "bin" else "" in find_best_compiler (List.map quote (List.map (Filename.concat path) commands)) let get_home () = try Sys.getenv "OCAML_HOME" with Not_found -> "" let expand_includes = let split = Str.split (Str.regexp " +") in fun compact -> if String.length compact > 0 then ("-I " ^ (String.concat " -I " (split compact))) else ""  let ocamlc ()   = match find_tool `BEST_OCAMLC (get_home ()) with Some x -> x | _ -> failwith "Cannot find 'ocamlc'" let ocamlopt () = find_tool `BEST_OCAMLOPT (get_home ()) let ocamldep () = match find_tool `BEST_OCAMLDEP (get_home ()) with Some x -> x | _ -> failwith "Cannot find 'ocamldep'" let ocamldoc () = match find_tool `BEST_OCAMLDOC (get_home ()) with Some x -> x | _ -> failwith "Cannot find 'ocamldoc'" let ocaml ()    = match find_tool `OCAML (get_home ()) with Some x -> x | _ -> failwith "Cannot find 'ocaml'" let ocamllib () = Cmd.expand ~first_line:true ((ocamlc()) ^ " -where")  let ocaml_version ?(compiler=ocamlc()) () = Cmd.expand (compiler ^ " -v " ^ redirect_stderr)  let can_compile_native ?ocaml_home () = let result = ref false in let filename = Filename.temp_file "test_native" ".ml" in let ochan = open_out filename in begin try output_string ochan ("0"); close_out ochan with _ -> (close_out ochan) end; let outname = Filename.chop_extension filename in let outname = outname ^ (if Sys.os_type = "Win32" then ".exe" else "") in let compiler = match ocaml_home with | Some home -> find_tool `BEST_OCAMLOPT home | _ -> Some "ocamlopt" in match compiler with | Some compiler -> let cmd = sprintf "%s -o %s %s%s" compiler outname filename (if App_config.application_debug then redirect_stderr else "") in result := (Sys.command cmd) = 0; if Sys.file_exists filename then (Sys.remove filename); if Sys.file_exists outname then (Sys.remove outname); let cmi = outname ^ ".cmi" in if Sys.file_exists cmi then (Sys.remove cmi); let cmx = outname ^ ".cmx" in if Sys.file_exists cmx then (Sys.remove cmx); let obj = outname ^ ".o" in if Sys.file_exists obj then (Sys.remove obj); let obj = outname ^ ".obj" in if Sys.file_exists obj then (Sys.remove obj); if !result then begin let conf = kprintf Cmd.expand "%s -config" compiler in let re = Str.regexp "ccomp_type: \\(.*\\)\n" in if Str.search_forward re conf 0 >= 0 then begin Some (Str.matched_group 1 conf) end else Some "<unknown ccomp_type>" end else None; | _ -> None ;; end
module Dep = struct open Printf type dag = (string, string list) Hashtbl.t exception Loop_found of string let (^^) = Filename.check_suffix let redirect_stderr = if Sys.os_type = "Win32" then " 2>NUL" else " 2>/dev/null" let re1 = Str.regexp " ?:\\( \\|$\\)" let re3 = Str.regexp " " let split_nl = Str.split (Str.regexp "\n")  let replace_extension x = sprintf "%s.%s" (Filename.chop_extension x) (if x ^^ "cmi" then "mli" else if x ^^ "cmx" then "ml" else assert false);;  let ocamldep ?pp ?(with_errors=true) ?(verbose=true) target = let dir = Filename.dirname target in let redirect_stderr = if with_errors then "" else (if Sys.os_type = "Win32" then " 2>NUL" else " 2>/dev/null") in let command = sprintf "%s%s %s -native -slash -one-line %s %s %s" (Ocaml_config.ocamldep()) (match pp with Some pp when pp <> "" -> " -pp " ^ pp | _ -> "" ) (Ocaml_config.expand_includes dir) (match dir with "." -> "*.mli" | _ -> dir ^ "/" ^ "*.mli *.mli") (match dir with "." -> "*.ml" | _ -> dir ^ "/" ^ "*.ml *.ml") redirect_stderr in if verbose then (printf "%s\n%!" command); let ocamldep = Cmd.expand command in let table = Hashtbl.create 7 in let entries = split_nl ocamldep in List.iter begin fun entry -> match Str.split re1 entry with | key :: _ when key ^^ ".cmo" -> () | key :: [] -> Hashtbl.replace table key [] | [key; deps] -> let deps = Str.split re3 deps in Hashtbl.replace table key deps | _ -> eprintf "%s\n%s\n%!" command entry; assert false end entries; (table : dag);;  let find_dep ?pp ?(with_errors=true) ?(echo=true) target = let table = ocamldep ?pp ~with_errors ~verbose:echo target in let target = (Filename.chop_extension target) ^ ".cmx" in let anti_loop = ref [] in let result = ref [] in let rec find_chain target = if (List.mem target !anti_loop) && (not (List.mem target !result)) then (raise (Loop_found (String.concat " " (List.map replace_extension (target :: !anti_loop))))); anti_loop := target :: (List.filter (fun x -> x <> target) !anti_loop); try if not (List.mem target !result) then begin match Hashtbl.find table target with | [] -> result := target :: !result; | deps -> List.iter find_chain deps; result := target :: !result; end with Not_found ->  (kprintf failwith "Dep: %s" target) in find_chain target; List.rev (List.map replace_extension !result) ;;  let array_exists from p a = try for i = from to Array.length a - 1 do if p a.(i) then raise Exit done; false with Exit -> true  let reduce : dag -> unit = function table -> let rec (<-?-) x y = let deps = try Hashtbl.find table y with Not_found -> [] in (List.mem x deps) || (List.exists ((<-?-) x) deps) in let is_descendant =                              (<-?-) in let reduce ll = let stop = ref "" in let rec reduce' ll = let len = Array.length ll in if len <= 1 then ll else let fst = ll.(0) in if fst = !stop then ll else begin let len = len - 1 in if array_exists 1 (is_descendant fst) ll then begin let tail = Array.make len "" in Array.blit ll 1 tail 0 len; reduce' tail end else begin if !stop = "" then (stop := fst); Array.blit ll 1 ll 0 len; ll.(len) <- fst; reduce' ll end end in Array.to_list (reduce' (Array.of_list ll)) in Hashtbl.iter (fun key deps -> Hashtbl.replace table key (reduce deps)) table ;;  let dot_of_dag (dag : dag) = let buf = Buffer.create 1000 in Buffer.add_string buf "digraph {\n"; Hashtbl.iter begin fun key -> List.iter (kprintf (Buffer.add_string buf) "%S -> %S;\n" key) end dag; Buffer.add_string buf "}\n"; Buffer.contents buf;;  let find' target = let target = (Filename.chop_extension target) ^ ".cmx" in let deps = ocamldep target in let out = Hashtbl.create 17 in let rec add root = let deps = try Hashtbl.find deps root with Not_found -> [] in Hashtbl.replace out root deps; List.iter add deps in add target; reduce out; (out : dag);;  let analyze dag = let rdag = let t = Hashtbl.create 17 in Hashtbl.iter (fun key deps -> List.iter (fun d -> Hashtbl.add t d key) deps) dag; t in let rec chain x = try begin match Hashtbl.find_all rdag x with | parent :: [] -> let n_childs = try List.length (Hashtbl.find dag parent) with Not_found -> 0 in if n_childs = 1 then x :: (chain parent) else [x] | _ -> [x] end; with Not_found -> [] in let leaves = Hashtbl.fold begin fun key deps acc -> match deps with | [] -> (chain key) :: acc | _ -> acc end dag [] in let fleaves = List.flatten leaves in List.iter (Hashtbl.remove dag) fleaves; Hashtbl.iter begin fun key deps -> Hashtbl.replace dag key (List.filter (fun d -> not (List.mem d fleaves)) deps) end dag; leaves ;;   let find ?pp ?with_errors ?(echo=true) targets = let deps = List.map (find_dep ?pp ?with_errors ~echo) targets in let deps = List.flatten deps in List.rev (List.fold_left begin fun acc x -> if not (List.mem x acc) then x :: acc else acc end [] deps) ;;  let find_dependants = let re = Str.regexp "\\(.+\\.mli?\\) ?: ?\\(.*\\)" in let re1 = Str.regexp "\r?\n" in let re2 = Str.regexp " " in fun ~dirname ~modname ->  let dir = if dirname = Filename.current_dir_name then "" else (dirname ^ "/") in let cmd = sprintf "%s -modules -native %s*.ml %s*.mli%s" (Ocaml_config.ocamldep()) dir dir redirect_stderr in printf "%s (%s)\n%!" cmd modname; let ocamldep = Cmd.expand cmd in let entries = Str.split re1 ocamldep in let entries = List.map begin fun entry -> if Str.string_match re entry 0 then begin let filename = Str.matched_group 1 entry in let modules = Str.matched_group 2 entry in (filename, (Str.split re2 modules)) end else (assert false) end entries in let dependants = ref [] in let rec loop modname = List.iter begin fun (filename, modules) -> if List.mem modname modules then begin if not (List.mem filename !dependants) then begin dependants := filename :: !dependants; let prefix = Filename.chop_extension filename in let prefix_mli = prefix ^ ".mli" in if List.mem_assoc prefix_mli entries then (dependants := prefix_mli :: !dependants;); let mdep = String.capitalize prefix in ignore (loop mdep); end end end entries; !dependants in loop modname let find_dependants ~path ~modname = let dependants = List.map (fun dirname -> find_dependants ~dirname ~modname) path in List.flatten dependants ;; end
...
let targets = [
  ...
  "markup-editor", {
    ...
    package              = "lablgtk2,xml-light,zip";
    search_path          = ""; (* -I *)
    required_libraries   = "";
  ...
  "test-markup-editor", {
    ...
    package              = "lablgtk2.auto-init,xml-light";
    search_path          = "tests"; (* -I *)
  ...
  "test-compress", {
    ...
    compilation_bytecode = false;
    compilation_native   = true;
    toplevel_modules     = "tests/test_compress.ml";
    package              = "str,unix,xml-light,zip";
    search_path          = ""; (* -I *)
    required_libraries   = "";
    compiler_flags       = "";
  ...
  "test-png", {
    ...
    compilation_bytecode = false;
    compilation_native   = true;
    toplevel_modules     = "tests/test_png.ml";
    package              = "unix,str,xml-light,zip";
    search_path          = "tests"; (* -I *)
    required_libraries   = "";
    compiler_flags       = "-w y -g";
